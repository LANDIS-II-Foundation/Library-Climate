        ...
        public ClimateFileFormatProvider(string format)
        {
            ...
            switch (this.format.ToLower())
            {
                case "daily_temp-c_precip-mmday":  //was 'gfdl_a1fi' then ipcc3_daily
                    //temp data are in oC and precip is in mm. CFFP (climate file format provider, L62) converts them to cm.
                    break;

                case "monthly_temp-c_precip-mmmonth":  // was ipcc3_monthly
                    //temp data are in oC and precip is in mm. CFFP converts them to cm.
                    break;
                ...
                case "daily_temp-k_precip-mmday":               // U of Idaho data     
                    //Temp are in Kelvin and precip is in mm.   
                    this.TemperatureTransformation = ABS_ZERO;
                    break;
                default:
                    Climate.TextLog.WriteLine($"Error in ClimateFileFormatProvider: the given '{format}' file format is not supported.");
                    throw new ApplicationException($"Error in ClimateFileFormatProvider: the given '{format}' file format is not supported.");
            }
        }
    }
}
