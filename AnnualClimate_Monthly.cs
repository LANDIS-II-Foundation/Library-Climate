...
namespace  Landis.Library.Climate
{
    public class AnnualClimate_Monthly: AnnualClimate
    {
        ...
        public double[] MonthlyTemperature = new double[12];
        ...
        public double[] MonthlySpei = new double[12];
        ...
        {
            if (Climate.FutureMonthlySpei == null)
            {
                // initialize Spei datastructrues if null
                Climate.FutureMonthlySpei = new Dictionary<int, double[]>[Climate.ModelCore.Ecoregions.Count];      // [ecoregion index][year] -> monthly spei
                Climate.SpinupMonthlySpei = new Dictionary<int, double[]>[Climate.ModelCore.Ecoregions.Count];      // [ecoregion index][year] -> monthly spei
            }

            if (Climate.FutureMonthlySpei[ecoregion.Index] == null)
            {
                // calculate monthly Spei data for the ecoregion
                var scale = 1; // todo: enable SPEI scale as a configuration parameter
                Climate.FutureMonthlySpei[ecoregion.Index] = Spei.CalculateMonthlySpei(ecoregion, latitude, Climate.Phase.Future_Climate, scale);
                Climate.SpinupMonthlySpei[ecoregion.Index] = Spei.CalculateMonthlySpei(ecoregion, latitude, Climate.Phase.SpinUp_Climate, scale);
            }

            var speiData = spinupOrfuture == Climate.Phase.Future_Climate ? Climate.FutureMonthlySpei[ecoregion.Index] : Climate.SpinupMonthlySpei[ecoregion.Index];
                ...
                case "Monthly_AverageAllYears":
                    {
                        ...
                        this.MonthlySpei = Spei.AverageSpeiOverYears(speiData);
                ...
                case "Monthly_RandomYears":
                    {
                        ...
                        CalculateMonthlyData(ecoregion, monthlyData, actualTimeStep, latitude);
                        this.MonthlySpei = speiData[actualTimeStep];
                        break;
                    }
            ...
            if (this.MonthlyPET[0] == 0.0)
                this.MonthlyPET = CalculatePotentialEvapotranspirationThornwaite(MonthlyTemp, MonthlyDayLength, actualTimeStep); ;   // if minRH exists, then estimate as the average of min and max  
        ...
        private void CalculateMonthlyData(IEcoregion ecoregion, ClimateRecord[] monthlyClimateRecords, int actualYear, double latitude)
        {
            ...
            for (int mo = 0; mo < 12; mo++)
            {
                ...
                this.MonthlyMinRH[mo] = monthlyClimateRecords[mo].AvgMinRH;
                this.MonthlyMaxRH[mo] = monthlyClimateRecords[mo].AvgMaxRH;
                this.MonthlyRH[mo] = (this.MonthlyMinRH[mo] + this.MonthlyMaxRH[mo]) / 2.0;
                this.MonthlySpecificHumidity[mo] = monthlyClimateRecords[mo].AvgSpecificHumidity;
        ...
        private ClimateRecord[] AnnualClimate_AvgMonth(IEcoregion ecoregion, double latitude)
        {
            ...
            for (int mo = 0; mo < 12; mo++)
            {
                ...
                foreach (var yearMonthlyRecords in timestepData.Values)
                {
                    ...
                    monthlyVarTemp += yearMonthlyRecords[ecoregion.Index][mo].VarTemp;
                    monthlyVarPpt += yearMonthlyRecords[ecoregion.Index][mo].VarPpt;
                    ...
                    monthlyMinRH += yearMonthlyRecords[ecoregion.Index][mo].AvgMinRH;
                    monthlyMaxRH += yearMonthlyRecords[ecoregion.Index][mo].AvgMaxRH;
                    monthlySH += yearMonthlyRecords[ecoregion.Index][mo].AvgSpecificHumidity;
                ...
                if (yearCount > 0)
                {
                    ...
                    monthlyData[mo].VarTemp = monthlyVarTemp / yearCount;
                    monthlyData[mo].StdDevTemp = Math.Sqrt(monthlyVarTemp / yearCount);
                    monthlyData[mo].VarPpt = monthlyVarPpt / yearCount;
        ...
        private ClimateRecord[] AnnualClimate_From_AnnualClimate_Daily(IEcoregion ecoregion, double latitude, Climate.Phase spinupOrfuture, int timeStep, int timeStepIndex)
        {
            ...
            for (int mo = 0; mo < 12; mo++)
            {
                ...
                monthlyData[mo].VarTemp = monthlyVarTemp / nDays;
                //monthlyData[mo].StdDevTemp = Math.Sqrt(monthlyVarTemp / nDays);
                //monthlyData[mo].VarPpt = monthlyPptVarTemp / nDays;
        ...
        public static double[] CalculatePotentialEvapotranspirationThornwaite(double[] monthlyTemp, double[] monthlyDayLengthInSec, int actualYear)
        {
            ...
            for (var i = 0; i < 12; i++)
            {
                ...
                var avgTemp = monthlyTemp[i]; // (annualClimate[i].AvgMinTemp + annualClimate[i].AvgMaxTemp) / 2.0;
                avgTemp = Math.Max(0.0, avgTemp);
                var heatIndexpermonth = Math.Pow((avgTemp / 5.0), 1.514);
            ...
            var monthlyPET = new double[12];
            for (var month = 0; month < 12; month++)
            {

                var meanDayLength = monthlyDayLengthInSec[month] / 3600.0;  //take day length from above in sec and convert to hours
                var totalDays = (DaysInMonth(month, actualYear));
                var avgTemp = monthlyTemp[month];

                avgTemp = Math.Max(0.0, avgTemp);
                
                //alpha term in Thornwaite equation that depends solely on the heat index
                                                                           
                monthlyPET[month] = (16.0 * (meanDayLength / 12.0) * (totalDays / 30.0) * (Math.Pow(((10.0 * avgTemp )/ heatIndex), alpha))/10.0);// equation by Thornthwaite divided by 10 to get cm/month
                //Climate.TextLog.WriteLine("Year={0}, Month={1}, PET={2:0.00}.", this.Year, month, monthlyPET[month]);
